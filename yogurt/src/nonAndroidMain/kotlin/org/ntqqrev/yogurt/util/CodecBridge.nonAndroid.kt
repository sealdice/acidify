...
internal actual suspend fun codecGetImageInfo(input: ByteArray): ImageInfo =
    org.ntqqrev.acidify.codec.getImageInfo(input).toMilkyImageInfo()
...
internal actual suspend fun codecGetVideoInfo(videoSource: MediaSource): VideoInfo =
    org.ntqqrev.acidify.codec.getVideoInfo(videoSource.readByteArray()).toMilkyVideoInfo()
...
private fun org.ntqqrev.acidify.codec.ImageInfo.toMilkyImageInfo() = ImageInfo(
    format = when (format) {
...
private fun org.ntqqrev.acidify.codec.VideoInfo.toMilkyVideoInfo() = VideoInfo(
    width = width,
    height = height,
    duration = duration,
)
