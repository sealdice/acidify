...
fun discoverCurlCommand(configuredPath: String = ""): String? {
    if (configuredPath.isNotBlank()) {
        return configuredPath
    }

    val candidateNames = listOf("curl", "curl.exe")
    val candidates = LinkedHashSet<String>()
    ...
    candidates += "/system/bin/curl"
    candidates += candidateNames
    return candidates.firstOrNull(::isCurlUsable)
}

private fun isCurlUsable(command: String): Boolean {
    val result = executeCommand(command, "--version")
    return result.errorCode == 0 && listOf(result.stdout, result.stderr).any { it.startsWith("curl ") }
}
