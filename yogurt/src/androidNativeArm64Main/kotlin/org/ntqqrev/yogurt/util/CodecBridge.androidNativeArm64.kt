...
internal actual suspend fun codecGetImageInfo(input: ByteArray): ImageInfo = commonImageInfoImpl(input)
...
internal actual suspend fun codecGetVideoInfo(videoSource: MediaSource): VideoInfo {
    ...
    return VideoInfo(0, 0, Duration.ZERO)
}
...
private fun commonImageInfoImpl(data: ByteArray): ImageInfo? = when {
    data.size < 12 -> null
